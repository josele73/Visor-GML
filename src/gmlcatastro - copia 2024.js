...
function extraeTag(lineas){   
    ...
    for(var i = 0; i < lineas.length; i++)
        ...
        */
        ...
        //console.log(ArrayListadoPuntos[0]);
...
function extraerPuntos(p){  //se llama desde extraerTAG
    ...
    //console.log(puntos[0],puntos[1]);
     ...
     puntitosOk=[]
     k=0;
    for (x=0;x<puntos.length;x++){  //Aqui quitamos los espacios en blanco cuando hay muchos
        if (puntos[x]!=""){
            puntitosOk[k]=puntos[x];
            k=k+1;}
            //puntos.splice(x, 1);
            
    }
    //console.log(puntos[2],puntos[3]);
   
    //console.log(puntitosOk);
    puntos=puntitosOk;
    totalPuntos=(puntos.length/2);
